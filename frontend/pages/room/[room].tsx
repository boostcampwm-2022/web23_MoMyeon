...
export default function Room({ roomName }: any) {
  ...
  useEffect(() => {
    ...
    socketRef.current.on("producer-closed", ({ remoteProducerId }: any) => {
      ...
      setRemoteVideoList((prev) =>
        prev.filter((producerId: any) => producerId !== remoteProducerId)
      );
  ...
  const connectRecvTransport = async (
    ...
    await socketRef.current.emit(
      ...
      async ({ params }: any) => {
        ...
        remoteVideoRef.current.remoteProducerId.srcObject = new MediaStream([
          track,
        ]);
                ...
                {remoteVideoList?.map((remoteProducerId) => {
                  return (
                    <video
                      ...
                      autoPlay
                      ref={(element) =>
                        (remoteVideoRef.current.remoteProducerId = element)
                      }
                    />
                  );
                })}
